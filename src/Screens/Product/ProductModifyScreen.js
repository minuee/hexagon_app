...
import {SafeAreaView,ScrollView,View,StyleSheet,Alert,Dimensions,RefreshControl,PixelRatio,TouchableOpacity,BackHandler,TextInput,KeyboardAvoidingView,Animated,Image as NativeImage} from 'react-native';
...
class ProductModifyScreen extends Component {
    constructor(props) {
        super(props);
    ...
    getBaseData = async(product_pk) => {
        ...
        try {            
            ...
            if ( returnCode.code === '0000'  ) {
                const productDetail = returnCode.data.productDetail;
                const baseByte = 10*1024*1024;
                let modPhotoarray =[];
                let tmpattachFileSize = 0;
                if ( !CommonUtil.isEmpty(productDetail.detail_img1)) {
                    modPhotoarray.push({
                        uri: DEFAULT_CONSTANTS.defaultImageDomain + productDetail.detail_img1,
                        isDatabaseFile : true,
                        fileSize: baseByte
                    })
                    tmpattachFileSize = tmpattachFileSize+baseByte;
                }
                if ( !CommonUtil.isEmpty(productDetail.detail_img2)) {
                    modPhotoarray.push({
                        uri: DEFAULT_CONSTANTS.defaultImageDomain + productDetail.detail_img2,
                        isDatabaseFile : true,
                        fileSize: baseByte
                    })
                    tmpattachFileSize = tmpattachFileSize+baseByte;
                }
                if ( !CommonUtil.isEmpty(productDetail.detail_img3)) {
                    modPhotoarray.push({
                        uri:DEFAULT_CONSTANTS.defaultImageDomain + productDetail.detail_img3,
                        isDatabaseFile : true,
                        fileSize: baseByte
                    })
                    tmpattachFileSize = tmpattachFileSize+baseByte;
                }
                if ( !CommonUtil.isEmpty(productDetail.detail_img4)) {
                    modPhotoarray.push({
                        uri: DEFAULT_CONSTANTS.defaultImageDomain + productDetail.detail_img4,
                        isDatabaseFile : true,
                        fileSize: baseByte
                    })
                    tmpattachFileSize = tmpattachFileSize+baseByte;
                ...
                this.setState({
                    ...
                    thumbnail_img : DEFAULT_CONSTANTS.defaultImageDomain + productDetail.thumb_img,
                    uploadImageThumb : null,
                    attachFileSize : tmpattachFileSize,
                    photoarray: modPhotoarray,
                    uploadImagaArray01 : productDetail.detail_img1,
                    uploadImagaArray02 : productDetail.detail_img2,
                    uploadImagaArray03 : productDetail.detail_img3,
                    uploadImagaArray04 : productDetail.detail_img4,
                    use_yn : productDetail.use_yn,
                    can_point : productDetail.can_point,
                    is_nonPoint : productDetail.is_nonpoint,
                    formCategoryPk : productDetail.category_pk,
                    formProductName : productDetail.product_name,
                    formPriceOne : productDetail.each_price,       
                    formPriceBox : productDetail.box_price,
                    formPriceBoxEach : productDetail.box_unit,
                    formPriceTon : productDetail.carton_price,
                    formPriceTonEach : productDetail.carton_unit,
                    formEventPriceOne : productDetail.event_each_price,    
                    formEventEachStock : productDetail.event_each_stock,
                    formEventPriceBox : productDetail.event_box_price,
                    formEventPriceBoxEach : productDetail.event_box_unit,
                    formEventBoxStock : productDetail.event_box_stock,
                    formEventPriceTon : productDetail.event_carton_price,
                    formEventPriceTonEach : productDetail.event_carton_unit,
                    formEventCartonStock : productDetail.event_carton_stock,
                    formMaterial : productDetail.material,
                })
            }else{
    ...
    singleLocalImageUpload = async(newImageadata,index = 0) => {
        if ( newImageadata.isDatabaseFile ) {
            return true;
        }else {
            try {
    ...
    registData = async() => {
        ...
        const Original_thumbnailImage = this.state.thumbnail_img.replace(DEFAULT_CONSTANTS.defaultImageDomain,"");
        let thumbnailImage = {data:null};
        if ( this.state.isNewImage ) {
            ...
            thumbnailImage = await CommonUtil.SingleImageUpload(this.props.userToken.apiToken,newImageadata,'product');
        }
        if ( this.state.isNewImage === false || ( this.state.isNewImage && !CommonUtil.isEmpty(thumbnailImage.data) )) {
            const promises = this.state.photoarray.map((element,index) => this.singleLocalImageUpload(element,parseInt(index+1)));
            await Promise.all(promises);
            let returnCode = {code:9998};
            try {            
                const url = DEFAULT_CONSTANTS.apiAdminDomain + '/cms/product/modify/'+this.state.product_pk;
                const token = this.props.userToken.apiToken;
                let sendData = {
                    product_pk : this.state.product_pk,
                    can_point : this.state.can_point,
                    use_yn :  this.state.use_yn,
                    category_pk : this.state.formCategoryPk,
                    ...
                    thumb_img : !CommonUtil.isEmpty(thumbnailImage.data) ? thumbnailImage.data : Original_thumbnailImage,
    ...
    changeProfile = async(cropit, circular = false, mediaType = 'photo') => {
        ...
        .then((response) => {            
            this.setState({thumbnail_img : response.path,isNewImage : true,image: response});
        })
        .catch((e) => {            
        ...
        return (
            <View style={{flex:1,flexGrow:1,flexDirection:'row'}}>
                ...
                }
                {
                    ...
                    <TouchableOpacity onPress={() => this.handleChoosePhoto()} style={styles.imageOuterWrap2}>
                        <NativeImage
                ...
                <View style={ styles.container }>
                    ...
                    >
                        <View style={{flex:1,backgroundColor:'#fff',padding:15}}>
                            <View style={{padding:20,marginBottom:20,alignItems:'center'}}>
                                <TouchableOpacity 
                            ...
                            <View style={{paddingHorizontal:5,marginTop:10}}>
                                <CustomTextR style={{fontSize:PixelRatio.roundToNearestPixel(DEFAULT_TEXT.fontSize11),color:'#666'}}>
                                    1개이상 필수입력
                                </CustomTextR>                                
                            </View>  
                            <View style={{paddingHorizontal:5,marginTop:20,flexDirection:'row',alignItems:'center'}}>
                                <CustomTextR style={{fontSize:PixelRatio.roundToNearestPixel(DEFAULT_TEXT.fontSize11),color:'#555'}}>
                                    이벤트 정보
                                </CustomTextR>
                                <CustomTextR style={[CommonStyle.requiredText2,{fontSize:PixelRatio.roundToNearestPixel(DEFAULT_TEXT.fontSize11)}]}>
                                    {' * 필요시에만 등록'}
                                </CustomTextR>
                            </View>
                            <View style={{paddingHorizontal:5,paddingVertical:10,flexDirection:'row'}}>
                            ...
                            <View style={[styles.cellDefaultRowWrap,{marginTop:0}]}>
...
export default connect(mapStateToProps,null)(ProductModifyScreen);
