...
class ModifyScreen extends Component {
    ...
    getBaseData = async(even_pk) => {
        ...
        try {            
            ...
            console.log('returnCode',returnCode.data.eventDetail)   
            if ( returnCode.code === '0000'  ) {
                this.setState({
                    eventType : returnCode.data.eventDetail.event_gubun,    
                    thumbnail_img :returnCode.data.eventDetail.event_img,    
                    formTitle : returnCode.data.eventDetail.title,            
                    formActionStart : CommonFunction.convertUnixToDate(returnCode.data.eventDetail.start_dt,'YYYY-MM-DD'),
                    formActionEnd : CommonUtil.isEmpty(returnCode.data.eventDetail.end_dt) ? currentDate : CommonFunction.convertUnixToDate(returnCode.data.eventDetail.end_dt,'YYYY-MM-DD'),    
                    selectTime : CommonFunction.convertUnixToNewDate(returnCode.data.eventDetail.start_dt),
                    formStartHour : CommonFunction.convertUnixToDate(returnCode.data.eventDetail.start_dt,'HH'),
                    formStartMinute : CommonFunction.convertUnixToDate(returnCode.data.eventDetail.start_dt,'mm'),
                    formEndHour : CommonUtil.isEmpty(returnCode.data.eventDetail.end_dt) ? 0 : CommonFunction.convertUnixToDate(returnCode.data.eventDetail.end_dt,'HH'),
                    formEndMinute : CommonUtil.isEmpty(returnCode.data.eventDetail.end_dt) ? 0 : CommonFunction.convertUnixToDate(returnCode.data.eventDetail.end_dt,'MM'),
                    formEventGoods : CommonUtil.isEmpty(returnCode.data.eventDetail.product_array) ? [] : returnCode.data.eventDetail.product_array,
                })
            }else{
    ...
    async UNSAFE_componentWillMount() {
        ...
        if ( !CommonUtil.isEmpty(this.props.extraData.params.screenData)) {
            ...
            this.setState({
                event_pk : this.props.extraData.params.screenData.event_pk
            })
        }else{
    ...
    setBirthday = async( datew ) => {
        ...
        //console.log('datew.getHours()', datew.getHours());
        //console.log('datew.getMinutes()', datew.getMinutes());
        if ( this.state.showModalType2 === 1 ) {
            ...
            this.setState({
                ...
                formEndMinute : datew.getMinutes() < 10 ? '0' + datew.getMinutes() : datew.getMinutes(),
            });
        } 
    }
    ...
    cancleRegist = () => {
        Alert.alert(
            ...
            "이벤트 중지하시겠습니까?",
            [
                {text: '네', onPress: () => this.registData('stop')},
    ...
    registData = async(mode = null) => {
        ...
        try {            
            ...
            const url = DEFAULT_CONSTANTS.apiAdminDomain + '/cms/event/stop/' + this.state.event_pk;
            // console.log('url',url) 
            const token = this.props.userToken.apiToken;
            ...
            if ( CommonUtil.isEmpty(this.state.formActionEnd)) {
                ...
                eDate2 = CommonFunction.convertDateToUnix(eDate);
            }
            if ( mode === 'stop') {
                eDate2 = moment().unix();
            }
            ...
            returnCode = await apiObject.API_updateCommon(this.props,url,token,null);          
    ...
    setProductStock = async(val, index ) => {
        if ( val == 0) {
            CommonFunction.fn_call_toast('최소 1개이상 선택해주세요.',2000);
            return;
        }else{
            this.state.formEventGoods[index].stock = val;
            this.setState({moreLoading:false});

            //console.log('this.state.formEventGoods',this.state.formEventGoods)
        }
    }
            ...
            return(
                <SafeAreaView style={ styles.container }>
                                ...
                                <View style={styles.productArrayWrap}>
                                {
                                    ...
                                    return (
                                        <View key={index} style={styles.boxSubWrap}>
                                            <TouchableOpacity 
                                                onPress={()=>this.moveDetail({product_pk:item.product_pk})}
                                            ...
                                            <View style={styles.boxStockWrap}>
                                                { this.state.eventType === 'LIMIT5' &&
                                                <TextInput     
                                                    keyboardType={'number-pad'}                               
                                                    style={[styles.inputBlank,CommonStyle.defaultOneWayFormAlignRight]}
                                                    value={CommonUtil.isEmpty(item.stock) ? 0:item.stock.toString()}
                    ...
                    <View style={CommonStyle.bottomButtonWrap}>
                        ...
                        >
                            <CustomTextB style={CommonStyle.bottomMenuOnText}>이벤트중지</CustomTextB>
                        </TouchableOpacity>
                        <TouchableOpacity 
