...
class PopEventRegistScreen extends Component {
    constructor(props) {
        super(props);
    ...
    getBaseData = async() => {
        ...
        try {            
            ...
            if ( returnCode.code === '0000'  ) {
                ...
                let reArray = await arrayTmp.map(({ event_pk, title }) => ({ id: event_pk, name: title }));
                this.setState({
                    eventList : reArray,
    ...
    registData = async() => {
        ...
        let returnCode = {code:9998};     
        try {            
            const url = DEFAULT_CONSTANTS.apiAdminDomain + '/cms/popevent/regist';           
            const token = this.props.userToken.apiToken;
            ...
            let sendData = {
                ...
                tarket_pk : this.state.inlinkType === 'EVENT' ? this.state.formEventPk : this.state.target_data.product_pk                
            }            
    ...
    selectFilter = async(filt) => {    
        this.setState({formEventPk:filt})
    }
        ...
        if ( this.state.loading ) {
            ...
            return(
                <View style={styles.container}>
                    ...
                    <TouchableOpacity style={CommonStyle.bottomButtonWrap} onPress={()=> this.registPosting()}>
                        <View  style={styles.menuOnBox}>
...
export default connect(mapStateToProps,null)(PopEventRegistScreen);