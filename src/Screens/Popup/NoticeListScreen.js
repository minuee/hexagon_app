...
class NoticeListScreen extends Component {
    ...
    moreDataUpdate = async( baseData , addData , gubun = null) => {     
        if ( gubun === null ) {
            let newArray = await baseData.concat(addData.data.nowPopList);  
            this.setState({            
                moreLoading : false,
                loading : false,
                noticeList : newArray,
            ...
            let newArray = await baseData.concat(addData.data.stopPopList);  
            this.setState({            
                moreLoading : false,
                loading : false,
    ...
    getBaseData = async(currentpage,morePage = false) => {
        ...
        try {            
            const url = DEFAULT_CONSTANTS.apiAdminDomain + '/cms/popup/list/now?page=' + currentpage + '&paginate='+DefaultPaginate;
            const token = this.props.userToken.apiToken;
            let sendData = null;
            returnCode = await apiObject.API_getPageList(this.props,url,token,sendData);          
            //console.log('getBaseDatareturnCode2',returnCode.data.nowPopList)   
            if ( returnCode.code === '0000'  ) {
                ...
                if ( morePage ) {
                    ...
                    this.setState({
                        ...
                        noticeList : CommonUtil.isEmpty(returnCode.data.nowPopList) ? [] : returnCode.data.nowPopList,
    ...
    getBaseDataOld = async(currentpage,morePage = false) => {
        ...
        let returnCode = {code:9998};
        try {            
            const url = DEFAULT_CONSTANTS.apiAdminDomain + '/cms/popup/list/stop?page=' + currentpage + '&paginate='+DefaultPaginate;
            const token = this.props.userToken.apiToken;
            let sendData = null;
            returnCode = await apiObject.API_getPageList(this.props,url,token,sendData);          
            //console.log('getBaseDataOldreturnCode',returnCode)   
            if ( returnCode.code === '0000'  ) {
                this.setState({currentPage : returnCode.currentPage})
                if ( morePage ) {
                    ...
                    this.setState({
                        ...
                        noticeListOld : CommonUtil.isEmpty(returnCode.data.stopPopList) ? [] : returnCode.data.stopPopList,
    ...
    moveDetail = async(item,mode) => {
        ...
        if ( mode === 'A' ) {
            this.props.navigation.navigate('PopNoticeModifyStack',{
                screenData:item
            })
        }else{
            ...
            <SafeAreaView style={ styles.container }>
                ...
                >
                    <View style={styles.boxWrap}>
                        <CustomTextM style={{fontSize:PixelRatio.roundToNearestPixel(DEFAULT_TEXT.fontSize15)}}>현재({CommonFunction.currencyFormat(this.state.totalCount)})</CustomTextM>
                    </View> 
                    {
                        this.state.noticeList.length === 0 ? 
                        <View style={CommonStyle.emptyWrap} >
                            <CustomTextR style={CommonStyle.dataText}>진행중인 팝업공지가 없습니다.</CustomTextR>
                        </View>
                        :
                    ...
                    <View style={styles.boxWrap}>
                        <CustomTextM style={{fontSize:PixelRatio.roundToNearestPixel(DEFAULT_TEXT.fontSize15)}}>지난({CommonFunction.currencyFormat(this.state.totalCountOld)})</CustomTextM>
                        
                    </View> 
                    {
                        this.state.noticeListOld.length === 0 ? 
                        <View style={CommonStyle.emptyWrap} >
                            <CustomTextR style={CommonStyle.dataText}>만료된 팝업공지가 없습니다.</CustomTextR>
                        </View>
                        :
