...
class BannerRegistScreen extends Component {
   ...
   setCategory = async(arr) => {
    ...
    registData = async() => {
        ...
        let returnCode = {code:9998};     
        try {            
            const url = DEFAULT_CONSTANTS.apiAdminDomain + '/cms/banner/regist';
           // console.log('url',url) 
            const token = this.props.userToken.apiToken;
            let sendData = {
    ...
    bannerRegist = async() => {
        if ( CommonUtil.isEmpty(this.state.image)) {
            ...
            //console.log('this.state.inlinkTarket',this.state.target_data)   
            if( this.state.linkType === 'OUTLINK' && CommonUtil.isEmpty(this.state.inlinkTarket)) {
                CommonFunction.fn_call_toast('브라우저호출시에는 이동한 주소를 입력해주세요',2000);
                return true;  
            }else if( this.state.linkType === 'INLINK' && this.state.inlinkType === 'PRODUCT' && this.state.inlinkTarket === 0 ) {
                CommonFunction.fn_call_toast('상품을 선택해주세요',2000);
                return true;  
            }else if( this.state.linkType === 'INLINK' && this.state.inlinkType === 'CATEGORY' && this.state.inlinkTarket === 0 ) {
                CommonFunction.fn_call_toast('카테고리를선택해주세요',2000);
                return true;  
            }else if( this.state.linkType === 'INLINK' && this.state.inlinkType === 'EVENT'&& this.state.inlinkTarket === 0 ) {
...
export default connect(mapStateToProps,mapDispatchToProps)(BannerRegistScreen);